...
def getPacketStruct(msgID):
   ...
   Note: all messages with data packets should be accounted for here, but there are almost certainly data entry errors, so some may be missing or incorrect. Few of them have been tested """
    ...
    if msgID in [0x07D1, 0x07D3, 0x07E8, 0x07EA, 0x0875]:
        return '<H'
    # 2 words
    elif msgID in [0x042C, 0x04B3, 0x04B5, 0x04FB, 0x04FD, 0x0643, 0x0645, 0x0646, 0x0648, 0x0652, 0x0654, 0x07D0,
                   0x0651, 0x0609, 0x0611, 0x07E7]:
        return '<HH'
    # 3 words
    ...
    # 1 word + 3 longs
        ...
        raise Exception("Message " + hex(
            msgID) + " has a variable data packet structure due to the use of submessages, which hasn't been implemented yet")
    else:
        raise Exception( "Message " + hex(
            msgID) + " does not have a packet structure specified. Please check the documentation for this messageID")
...
BAY_TYPE_SERIAL_PREFIXES = ["70", "71", "73",
                            "94"]  # The first two digits of the serial numbers of the controllers which use the bay type architecture
# Channel IDs
CHANNEL_1 = 0x01
CHANNEL_2 = 0x02
CHANNEL_3 = 0x03
ALL_CHANNELS = [CHANNEL_1, CHANNEL_2, CHANNEL_3]
...
def getMotorScalingFactors(controllerType, stageType):
    ...
    if controller == "TDC001" or controller == "KDC101":
        ...
        encCnt = {"DRV001": 51200, "DRV013": 25600, "DRV014": 25600, "DRV113": 20480, "DRV114": 20480,
                  "FW103": 71 / 0.998, "NR360": 4693 / 0.999}
        return {"position": encCnt[stage], "velocity": encCnt[stage], "acceleration": encCnt[stage]}
    elif controller in ["BSC20X", "MST602","KST101"]:
        encCnt = {"DRV001": 819200,"DRV208": 819200, "DRV013": 409600, "DRV014": 409600, "DRV113": 327680, "DRV114": 327680,
                  "FW103": 1138 / 1.0002, "NR360": 75091 / 0.99997}
...
# Constants for piezo controller
