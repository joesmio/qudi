...
class MicrowaveSmbv(Base, MicrowaveInterface):
    ...
    def get_frequency(self):
        ...
        if 'cw' in mode:
            ...
            step = 2*float(self._connection.query(':SWE:STEP?'))
    ...
    def set_sweep(self, start=None, stop=None, step=None, power=None):
        ...
        if (start is not None) and (stop is not None) and (step is not None):
            self._connection.write(':SWE:FREQ:MODE AUTO') # changed to auto sweep JS 06/02/19
            self._connection.write('TRIG:FSW:SOUR SING')
            self._connection.write('SOUR:FREQ:MODE SWE') # we don't want a power sweep do we?
            self._connection.write('SOUR:SWE:FREQ:EXEC')
            ...
            self._connection.write(':SWE:STEP:LIN {0:f}'.format(step/2))
            self._connection.write('*WAI')
