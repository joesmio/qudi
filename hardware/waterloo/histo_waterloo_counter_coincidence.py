...
class mysocket:
    ...
    def __init__(self, sock=None, channels=[0], biases=[], delays=[], coincidences=[], window=0, histogram_channels=[1],
...
class WaterlooCounter2(Base, SlowCounterInterface):
    ...
    def on_activate(self):
       ...
       # self.log.info('channel list is {0}'.format(self._chan_list))
        ...
        self.ms = mysocket(sock=None, channels=self._chan_list, biases=[-0.8,1.2,1.2,1.5,1.1], delays=[0,2,5,10,0], coincidences=self._coincidence,
                           window=window,
    ...
    def set_up_counter(self,
        ...
        self.ms.channels =  [0,1]#counter_channels
    ...
    def get_counter(self, samples=None):
        ...
        found = 0
        counts_out = np.empty(
            (samples,len(self.get_counter_channels())),
            dtype=np.uint32)
        ...
        while found < samples:
            ...
            if 'counts' in decrypted['type']:
                ...
                histo = decrypted['histogram_counts']
                counts0 = counts[0]
                ...
                if counts0 != counts0 or counts0  <= 0:
                    ...
                    self.histo_file.write(str(histo).replace('[','').replace(']','') + '\n')
                    counts_out[samples-1] = counts_out_col
