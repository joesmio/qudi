...
class MotorApt(Base, MotorInterface):
    ...
    def on_activate(self):
        ...
        # for position and velocity, additional scale factors as per https://www.thorlabs.com/Software/Motion%20Control/APT_Communications_Protocol.pdf
        #self.pos_sf = 1e3* (360/1.8)* 2048 /0.5
        #self.vel_sf = self.pos_sf * 53.69
        #self.acc_sf = self.pos_sf / 90.9


        #TODO: read from dict


        #self.kcube_y = AptMotor(hwser=26001815)
        #self.kcube_x = AptMotor(hwser=26001817)
        #self.kcube_z = AptMotor(hwser=26001403)

        '''
        15/05 new setup
        
        Qudi z is camera x (and more importantly piezo x)
        ...
        '''
        self.kcube_x = AptMotor(hwser=26001403)
        self.kcube_y = AptMotor(hwser=26001817)
        self.kcube_z = AptMotor(hwser=26001815)
    ...
    def move_abs(self, param_dict):
        ...
        if param_dict.get(self._x_axis.label) is not None:
            ...
            if not(constr['pos_min'] <= desired_pos <= constr['pos_max']):
                ...
                if abs(desired_pos - self._x_axis.pos) > 1e-7: # put a 100 nm tolerance on the stepper motor
                    self.kcube_x.setPosition(position = float((desired_pos+3e-3)*1e3))
                    self._make_wait_after_movement()
        ...
        if param_dict.get(self._y_axis.label) is not None:
            ...
            if not(constr['pos_min'] <= desired_pos <= constr['pos_max']):
                ...
                # y range is 3mm to 7 mm but direction is reversed
                if abs(desired_pos - self._y_axis.pos) > 1e-7:
                    self.kcube_y.setPosition(position = float((-desired_pos+8e-3) * 1e3))
                    self._make_wait_after_movement()
        ...
        if param_dict.get(self._z_axis.label) is not None:
            ...
            if not(constr['pos_min'] <= desired_pos <= constr['pos_max']):
                ...
                # z range is 3mm to 7 mm

                if abs(desired_pos - self._z_axis.pos) > 1e-7:
                    self.kcube_z.setPosition(position = float((desired_pos+3e-3) * 1e3))
                    self._make_wait_after_movement()
    ...
    def get_pos(self, param_list=None):
        ...
        if param_list is not None:
            ...
            if self._y_axis.label in param_list:
                self._y_axis.pos = (8-self.kcube_y.getPosition())*1e-3
    ...
    def get_pos_raw(self, param_list=None):
        ...
        if param_list is not None:
            ...
            if self._y_axis.label in param_list:
                pos[self._y_axis.label] =self.kcube_y.getPositionRaw()

            if self._z_axis.label in param_list:
                pos[self._z_axis.label] = self.kcube_z.getPositionRaw()
    ...
    def get_velocity(self, param_list=None):
        ...
        if param_list is not None:
            if self._x_axis.label in param_list:
                pos[self._x_axis.label] = self.kcube_x.GetVelocity()

            if self._y_axis.label in param_list:
                pos[self._y_axis.label] =self.kcube_y.GetVelocity()

            if self._z_axis.label in param_list:
                pos[self._z_axis.label] = self.kcube_z.GetVelocity()


        else:
            pos[self._x_axis.label] = self.kcube_x.GetVelocity()*1e-3
            pos[self._y_axis.label] = self.kcube_y.GetVelocity()*1e-3
            pos[self._z_axis.label] = self.kcube_z.GetVelocity()*1e-3
    ...
    def get_params(self, param_list=None):
        ...
        if param_list is not None:
            ...
            if self._y_axis.label in param_list:
                pos[self._y_axis.label] = self.kcube_y.getLimitParams()
                pos[self._z_axis.label].update(self.kcube_y.getHomeParams())

            if self._z_axis.label in param_list:
                pos[self._z_axis.label] = self.kcube_z.getLimitParams()
                pos[self._z_axis.label].update(self.kcube_z.getHomeParams())
            ...
            pos[self._x_axis.label].update(self.kcube_x.getHomeParams())
            pos[self._y_axis.label] = self.kcube_y.getLimitParams()
            pos[self._y_axis.label].update(self.kcube_y.getHomeParams())
            pos[self._z_axis.label] = self.kcube_z.getLimitParams()
            pos[self._z_axis.label].update(self.kcube_z.getHomeParams())
        ...
        try:
            ...
            pass
        try:
            pos[self._z_axis.label]['CW software limit'] = str((float(pos[self._z_axis.label]['CW software limit']) - 3) * 1e-3)
            pos[self._z_axis.label]['CCW software limit'] = str((float(pos[self._z_axis.label]['CCW software limit'])-3) * 1e-3)
        except KeyError:
            pass
        try:
            pos[self._y_axis.label]['CW software limit'] = str((8 - float(pos[self._y_axis.label]['CW software limit'])) * 1e-3)
            pos[self._y_axis.label]['CCW software limit'] = str((8 - float(pos[self._y_axis.label]['CCW software limit'])) * 1e-3)
        except KeyError:
            pass
    ...
    def update_encoder(self, param_list = None):
        ...
        if param_list is not None:
            if self._x_axis.label in param_list:
                desired_pos = self.kcube_x.UpdateEncoder(position =(param_list[self._x_axis.label]+ 3e-3) * 1e3)
                #print(desired_pos)

            if self._y_axis.label in param_list:
                desired_pos = self.kcube_y.UpdateEncoder(position=(8e-3 - param_list[self._y_axis.label]) * 1e3)
                #print(desired_pos)

            if self._z_axis.label in param_list:
                desired_pos  = self.kcube_z.UpdateEncoder(position=(param_list[self._z_axis.label] + 3e-3) * 1e3)
                #print(desired_pos)


    def reset_limit_switches(self):
        self.kcube_x.setHomeParams(0)
        self.kcube_y.setHomeParams(0)
        self.kcube_z.setHomeParams(0)
        self.kcube_x.SetLimitSwitchParams(0)
        self.kcube_y.SetLimitSwitchParams(0)
        self.kcube_z.SetLimitSwitchParams(0)
    ...
    def home(self,param_list = None):
        if param_list is not None:
            ...
            if self._y_axis.label in param_list:
                self.kcube_y.MoveHome(channel=0)

            if self._z_axis.label in param_list:
                self.kcube_z.MoveHome(channel=0)
    ...
    def jog_step(self, param_list = None):
        ...
        if param_list is not None:
            ...
            if self._y_axis.label in param_list:
                self.kcube_y.jog_step(param_list[self._y_axis.label])
    ...
    def jog_actual(self, param_list = None, positive =True):
        ...
        if param_list is not None:
            if positive is True:
                if self._x_axis.label in param_list:
                    self.kcube_x.jogforward()
                    self._x_axis.pos =  (self.kcube_x.getPosition()-3)*1e-3

                if self._y_axis.label in param_list:
                    self.kcube_y.jogforward()
                    self._y_axis.pos = (self.kcube_y.getPosition()-3)*1e-3

                if self._z_axis.label in param_list:
                    self.kcube_z.jogforward()
                    self._z_axis.pos = (self.kcube_z.getPosition()-3)*1e-3
            else:
                if self._x_axis.label in param_list:
                    self.kcube_x.jogback()
                    self._x_axis.pos = (self.kcube_x.getPosition() - 3) * 1e-3

                if self._y_axis.label in param_list:
                    self.kcube_y.jogback()
                    self._y_axis.pos = (self.kcube_y.getPosition() - 3) * 1e-3

                if self._z_axis.label in param_list:
                    self.kcube_z.jogback()
                    self._z_axis.pos = (self.kcube_z.getPosition() - 3) * 1e-3
