    ...
    def updateData(self):
        ...
        if self._counting_logic.module_state() == 'locked':
            ...
            for i, ch in enumerate(self._counting_logic.get_channels()):
                ...
                if i is 2:
                    factor = self.laser_factor
                else:
                    factor = 1

                if 0 < self._counting_logic.countdata_smoothed[i, -1] < 10:
                    label+=' {1}:, {0:,.6f}'.format(factor*self._counting_logic.countdata_smoothed[i, -1],ch)
                else:
                    label+= ' {1}: {0:,.0f}'.format(factor*self._counting_logic.countdata_smoothed[i,-1],ch)
            self._mw.count_value_Label.setText(label)
            x_vals = (
            ...
            for i, ch in enumerate(self._counting_logic.get_channels()):
                ...
                self.curves[2 * i].setData(y=factor*self._counting_logic.countdata[i], x=x_vals)
                self.curves[2 * i + 1].setData(y=factor*self._counting_logic.countdata_smoothed[i],
                                               x=x_vals
                                               )
                if ymax < factor*self._counting_logic.countdata[i].max() and self._trace_selection[i]:
                    ymax = factor*self._counting_logic.countdata[i].max()
                if ymin > factor*self._counting_logic.countdata[i].min() and self._trace_selection[i]:
                    ymin = factor*self._counting_logic.countdata[i].min()
