...
class PulsedMeasurementLogic(GenericLogic):
    ...
    def set_fast_counter_settings(self, settings_dict=None, **kwargs):
        ...
        if not counter_status >= 2 and not counter_status < 0:
            ...
            self.fastcounter().configure(self.__fast_counter_binwidth,
    ...
    def pause_pulsed_measurement(self):
        ...
        with self._threadlock:
            if self.module_state() == 'locked':
                ...
                self.sigMeasurementStatusUpdated.emit(False, True)

                # stop running so we can check signal JS 11/06/2019
            else:
    ...
    def _initialize_data_arrays(self):
        ...
        if self.__fast_counter_gates > 0:
           ...
           try:
                ...
                self.log.error('Check units! Memory error for {0} bins'.format(number_of_bins))
