...
class ODMRLogic(GenericLogic):
    ...
    def mw_sweep_on(self):
        ...
        if mode != 'list' and mode != 'sweep':
            self.log.error('Switching to list/sweep microwave output mode failed as mode is {0}'.format(mode))
        elif self.mw_scanmode == MicrowaveMode.SWEEP:
    ...
    def _start_odmr_counter(self):
        ...
        counter_status = self._odmr_counter.set_up_odmr(length = self.odmr_plot_x.size) # JS Edit to make it work in hardware
        if counter_status < 0:
    ...
    def _scan_odmr_line(self):
        ...
        with self.threadlock:
            ...
            self._mw_device.trigger() # JS 07/02/18 Trigger MW sweep each line
