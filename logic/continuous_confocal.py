...
#            time.sleep(0.01)
        ...
        # send resolution, min max etc through
    ...
    def start_scanner(self):
        ...
        scanner_status = self._scanning_device.set_up_scanner(res = self.xy_resolution, xrange = self.x_range, yrange = self.y_range)
    ...
    def _scan_line(self):
        ...
        #     self._scanning_device.arduino.cmd('scan\r\n')
        ...
        try:
            ...
            #new_image = self._scanning_device.scan_image()
            ...
            if self._zscan:
                if self.depth_img_is_xz:
                    self.depth_image[self._scan_counter, :, 3:3 + s_ch] += new_line
                else:
                    self.depth_image[self._scan_counter, :, 3:3 + s_ch] += new_line
                self.signal_depth_image_updated.emit()
            else:
                #self.xy_image[:, :, 3:3 + s_ch] = (new_image + self._scan_counter*self.xy_image[:, :, 3:3 + s_ch])/(1+self._scan_counter)
                self.xy_image[self._scan_counter, :, 3:3 + s_ch] += new_line
                self.signal_xy_image_updated.emit()
